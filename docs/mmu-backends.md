...
hiding it locally is not enough. `AfcBackend` sends `AFC_CLEAR_MESSAGE`. A
backend whose message is derived from live state has nothing to pop and leaves
the default alone.
...
`none` (the default) disables the panel entirely — no tab, no MMU code. For any
...
empty slot and pre-staging one is a real workflow. Set it false only where the
vendor hands metadata ownership elsewhere and would refuse the edit.
